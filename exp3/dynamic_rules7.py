...
class dynamic_rules(app_manager.RyuApp):
	...
	def install_path(self, src_dpid, dst_dpid, src_port, dst_port, ev, src, dst, pkt_ipv4, pkt_tcp):
		...
		# mid_path = self.long_path(src=src_dpid, dst=dst_dpid, count = self.count)
		mid_path = self.allPath(src=src_dpid, dst=dst_dpid)
		...
		for i in xrange(len(self.path) - 2, -1, -2):
			...
			self.add_flow(datapath_path, 100, match, actions, idle_timeout=5, hard_timeout=5)
	...
	def allPath(self, src, dst):
		if src not in self.src_links or self.src_links[src] is None:
			return None
		if dst not in self.src_links or self.src_links[dst] is None:
			return None
		tempResult = []
		resultList = []
		used = {}
		for linkSrc in self.src_links:
			for (tempSrc, tempDst) in self.src_links[linkSrc]:
				used[tempSrc] = used[tempDst] = False
		used[src] = True
		self.help(src, dst, used, tempResult, resultList)
		resultList.sort(key=lambda x: len(x))
		n = len(resultList)
		if (src, dst) not in self.count:
			self.count[(src, dst)] = -1
		self.count[(src, dst)] = (self.count[(src, dst)] + 1) % n
		return resultList[self.count[(src, dst)]]

	def help(self, src, dst, used, tempResult, resultList):
		if src == dst:
			resultList.append([i for i in tempResult])
			return
		for (temp_src, temp_dst) in self.src_links[src]:
			if used[temp_dst] is True:
				continue
			tempResult.append((src, self.src_links[src][(temp_src, temp_dst)][0]))
			tempResult.append((temp_dst, self.src_links[src][(temp_src, temp_dst)][1]))
			used[temp_dst] = True
			self.help(temp_dst, dst, used, tempResult, resultList)
			used[temp_dst] = False
			tempResult.pop(-1)
			tempResult.pop(-1)
		return
